...
## Read PRO-seq data.
ps_plus_path  <- "MCF7.40m.all_plus.bw"
ps_minus_path <- "MCF7.40m.all_minus.bw"
...
pred_val<- eval_reg_svm(gdm, asvm, inf_positions, ps_plus_path, ps_minus_path, batch_size= 10000, ncores=62)

final_data <- data.frame(inf_positions, pred_val)
options("scipen"=100, "digits"=4)
write.table(final_data, file="mcf7.E2_40m.predictions.bedGraph", row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
